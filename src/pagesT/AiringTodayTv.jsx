...
export default function AiringTodayTv() {
  ...
  const airingTodayTv = async (pageNumber) => {
    const get = await axios.get(
      `https://api.themoviedb.org/3/discover/tv?api_key=1b6ccfb407b0626e097c87368fba764e&language=en-US&sort_by=first_air_date.desc&with_genres=${state.state3}&first_air_date.gte=${sliderValues[0]}-01-01&first_air_date.lte=${sliderValues[1]}-${currentMonth}-${currentDay}&page=${pageNumber}`
    );
