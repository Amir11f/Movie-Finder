...
function SearchBoxN( props) {
    ...
    const searchMovie = async ()=>{
        const get = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=(your api key)&language=en-US&query=${getSearch}&page=1&include_adult=false`)
    ...
    const searchTv = async ()=>{
        const get = await axios.get(`https://api.themoviedb.org/3/search/tv?api_key=(your api key)&language=en-US&page=1&query=${getSearch}&include_adult=false`)
    ...
    <div className={`type ${props.searchBox? 'openSearch' : 'closeSearch'}`} >
            <div className="inType">
                <div className="inTypeInput">
                    <input placeholder='search for a movie, tv show'className='input1' type='text' onChange={(e)=>{setGetSearch(e.target.value)}} value={getSearch} onKeyDown={keydownfunction} ref={refInput}/>
                </div>
